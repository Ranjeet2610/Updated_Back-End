    ...
    if (req.body.description) {
        ...
        DB.user.findOne({userName:req.body.userName}).then((user) => {
            if(user){
                ...
                DB.betting.find({clientName:req.body.userName, eventID: req.body.eventID, marketType: 'match odds', status:"open"}).then((bets) => {
                    let T1TotalPL = 0;
                    let T2TotalPL = 0;
                    let T3TotalPL = 0;
                    bets.map(item => {
                                   ...
                                   console.log(userData);
                                    DB.user.findOne({userName: userData.master}).then(masterD => {
                                        ...
                                        if(e.runners.Teamthree != 'undefined'){
                                            let minVal = Math.min(parseFloat(e.runners.T2TotalPL), parseFloat(e.runners.T1TotalPL), parseFloat(e.runners.T3TotalPL));
                                            if(e.runners.Teamone == winnerSelectionId){
                                                ...
                                                userData.walletBalance = parseFloat(userData.walletBalance) + parseFloat(profit_loss) - parseFloat(minVal);
                                                userData.exposure = parseFloat(userData.exposure)  + parseFloat(minVal);
                                            ...
                                            let minVal = Math.min(parseFloat(e.runners.T2TotalPL), parseFloat(e.runners.T1TotalPL));
                                            if(e.runners.Teamone == winnerSelectionId){
                                                let sign = Math.sign(e.runners.T1TotalPL);
                                                let profit_loss;
                                                if(sign != -1){
